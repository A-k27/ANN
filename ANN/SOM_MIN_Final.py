...
# Initialize the SOM
som = MiniSom(grid_size[0], grid_size[1], input_len, sigma=2.0, learning_rate=0.001)

# Creating custom training function - decaying learning rate and radius
def train_som_with_decay(som, data, num_epoch=1000, decay_factor=10):
    ...
    for i in range(num_epoch):
        ...
        for x in data:
            winner = som.winner(x)  # Get the BMU (Best Matching Unit)
            som.update(x, winner, learning_rate, sigma)  # Update the weights
